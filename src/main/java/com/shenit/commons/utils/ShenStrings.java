...
public final class ShenStrings {
	 ...
	 * @return
	 */
	public static String joinWithSeperator(Object...objs){
		if(ShenArrays.isEmpty(objs)) return org.apache.commons.lang3.StringUtils.EMPTY;
		...
		List<String> items = Arrays.stream(objs)
				...
				.map(item ->  {
					if(item.getClass().isArray()){
						Object[] newObjs = Arrays.copyOf((Object[])item, len(item) + 1);
						newObjs[newObjs.length-1] = seperator;
						return joinWithSeperator(newObjs);
					}else {
						return str(item);
					}
		...
		return joinAll(items,seperator);
	}
	 ...
	 * @return
	 */
	public static String joinAll(List<?> list, char separator) {
		return joinAll(list,String.valueOf(separator));
	}
