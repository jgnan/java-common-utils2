...
public class ShenHttpCookie {
    ...
    public static ShenHttpCookie parse(String cookieStr){
        ...
        if(ShenArrays.isEmpty(keyVals)) return null;
        String key = ShenArrays.get(keyVals, 0);
        String val = ShenArrays.get(keyVals,1);
        ShenHttpCookie cookie = new ShenHttpCookie(key,val);
        String attribute;
        String attrValue;
        for(int i=2;i<keyVals.length;i+=2){
            attribute = StringUtils.trim(ShenArrays.get(keyVals,i));
            attrValue = StringUtils.trimToEmpty(ShenArrays.get(keyVals, i+1));
