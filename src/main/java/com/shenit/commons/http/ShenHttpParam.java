...
import com.shenit.commons.utils.ShenArrays;
...
public class ShenHttpParam extends TreeMap<String,List<Object>>{
    ...
    public ShenHttpParam addAll(Object... keysAndVals){
        if(ShenArrays.isEmpty(keysAndVals)) return this;
        String key, val;
        List<Object> vals;
        for(int i=0;i<keysAndVals.length;i+=2){
            key =DataUtils.toString(ShenArrays.get(keysAndVals,i));
            val = DataUtils.toString(ShenArrays.get(keysAndVals,i+1,StringUtils.EMPTY));
    ...
    public String getParam(String key,String delimiter){
        ...
        if(StringUtils.isEmpty(delimiter)) delimiter = ShenStrings.SEMICOLON;
        String[] headerVals = vals.stream()
            .map(val -> DataUtils.toString(val)).toArray(String[]::new);
        return ShenArrays.isEmpty(headerVals) ? StringUtils.EMPTY : StringUtils.join(headerVals, delimiter);
    }
